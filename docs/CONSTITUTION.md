...
- Any change to the system's data-flow, file structure, or layout mechanics must be preceded or accompanied by an update in `docs/architecture.md` or a new ADR (Architecture Decision Record) under `docs/decisions/`.
...
- Do not reinvent the wheel. Before creating a new custom button, card, or modal, search the `src/components/` directory for existing components that can be generalized or configured via props.

### 5. Keep the Project Modular
- Files must remain small, focused, and single-purpose.
- Avoid consolidating distinct modules or views into single files (e.g., do not build a massive, complex component in `App.tsx`; extract views to `src/pages/` and structural blocks to `src/components/layout/` or `src/navigation/`).
