  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8" id="full-profile-container">
      ...
      <div className="mb-8" id="profile-page-header">
        <h1 className="text-3xl font-extrabold tracking-tight text-text-heading">
          My Profile
        </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8" id="profile-content-layout">
        ...
        <div className="w-full lg:w-64 shrink-0" id="profile-tabs-sidebar">
          <div className="flex flex-row lg:flex-col overflow-x-auto lg:overflow-x-visible pb-3 lg:pb-0 gap-1 scrollbar-none border-b lg:border-b-0 lg:border-r border-card-border">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center space-x-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all whitespace-nowrap text-left w-full ${
                    isActive
                      ? 'text-link-primary bg-status-info-bg/50'
                      : 'text-text-muted hover:text-text-heading hover:bg-card-header-bg'
                  }`}
                  id={`profile-tab-btn-${tab.id}`}
                >
                  <Icon className={`h-4.5 w-4.5 ${isActive ? 'text-link-primary' : 'text-text-muted'}`} />
                  <span className="truncate">{tab.label}</span>
                  {isActive && (
                    <motion.div
                      layoutId="active-profile-tab-indicator"
                      className="absolute left-0 top-0 bottom-0 w-1 bg-link-primary rounded-r-md hidden lg:block"
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-0.5 bg-link-primary rounded-full lg:hidden"
                      transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                    />
                  )}
                </button>
        ...
        <div className="flex-1 min-w-0" id="profile-tab-content-panel">
          <div className="rounded-2xl border border-card-border/80 bg-card-bg p-6 md:p-8 shadow-sm">
            ...
            {activeTab === ProfileTab.CERTIFICATES && (
              <div id="tab-content-certificates" className="flex flex-col items-center justify-center min-h-[300px] text-center py-12">
                <div className="rounded-full bg-status-info-bg p-4 mb-4">
                  <CertificateIcon className="h-10 w-10 text-link-primary" />
                </div>
                <h4 className="text-base font-bold text-text-heading mb-1">
                  Certificates
                </h4>
            ...
            {activeTab === ProfileTab.BADGES && (
              <div id="tab-content-badges" className="flex flex-col items-center justify-center min-h-[300px]">
                <div className="rounded-full bg-status-warning-bg p-4 mb-4">
                  <Medal className="h-10 w-10 text-status-warning-text" />
                </div>
                <h4 className="text-base font-bold text-text-heading mb-1">
                  Badges & Achievements
                </h4>
