...
- **Components**: `src/features/content/components/ContentImportWizard.tsx`, `src/features/content/components/ScormPlayer.tsx`, `src/features/content/components/ScormPreviewPlayer.tsx`
- **Pages**: `src/features/management/pages/Management.tsx` (Curriculum & Lessons management tab), `src/features/assignments/pages/ContentManagement.tsx`, `src/features/lessons/pages/MyLessons.tsx` (Launch/Resume course play)
...
- **APIs**: `GET /api/content`, `GET /api/content/:id`, `POST /api/content/import`, `POST /api/content/:id/publish`, `POST /api/content/:id/archive`, `POST /api/content/:id/restore`, `GET /api/content/:id/download`, `GET /api/content/:contentGroupId/versions`, `POST /api/content-attempts/start`, `POST /api/content-attempts/:id/commit`, `GET /api/content-attempts/:instanceId`
- **Database**: `contents`, `lessons`, `content_tags`, `content_categories`, `content_attempts`, `user_assignment_instances` (Prisma & MariaDB schemas)
...
- **Routes**: `server/src/features/content/routes/content.routes.ts`, `server/src/features/content/routes/contentAttempts.routes.ts`, `/preview/content/:contentId`
...
- **Pages**: `src/features/organization/pages/UserGroupManagement.tsx` (Houses "Users" and "Profiles Data" tabs), `src/features/profiles/pages/FieldBuilder.tsx` (Profiles Category & Custom Field Builder nested under Administration card), `src/features/management/pages/Management.tsx` (Management Hub)
...
- **Pages**: `src/features/profiles/pages/Profile.tsx` (Notifications tab), global Navbar layout (`src/shared/components/layout/Navbar.tsx`), `src/features/notifications/pages/NotificationsHub.tsx` (consolidated Settings hub sub-tabs for Rules, Delivery Failures, Scheduled Notifications, and Email Templates), `src/features/notifications/pages/NotificationRuleManagement.tsx`, `src/features/notifications/pages/DeliveryFailures.tsx`, `src/features/notifications/pages/ScheduledNotificationManagement.tsx`, `src/features/notifications/pages/EmailTemplateManagement.tsx`
- **Services**:
...
- **APIs**:
  ...
  - `GET /api/notification-admin/rules` (list company notification rules)
  - `POST /api/notification-admin/rules` (create custom notification rule)
...
- **Routes**:
  - Client: Global Navbar bell trigger (flyout/modal), Settings/Profile `notifications` tab, Management hub `Notification Rules` view, Settings hub consolidated `Notifications` view with permission-gated sub-tabs (`rules`, `delivery-failures`, `scheduled`, `templates`)
