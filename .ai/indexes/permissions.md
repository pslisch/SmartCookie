...
| `theme` | `view` | View theme management dashboard, theme list, font library, inspect theme details, acquire test locks, dismiss activation failure alerts, and test themes | `GET /api/themes`, `GET /api/themes/:id`, `GET /api/fonts`, `GET /api/themes/:id/lock`, `POST /api/themes/:id/lock` (TEST), `POST /api/themes/:id/dismiss-failure` | Settings Theme tab, Theme Management dashboard, Theme test mode triggers, ThemeTestBanner |
| `theme` | `edit` | Create and edit draft themes, upload custom fonts, delete/replace fonts, acquire edit locks, modify color tokens, typography slots, and base font size | `POST /api/themes`, `PATCH /api/themes/:id`, `POST /api/fonts`, `DELETE /api/fonts/:id`, `POST /api/fonts/:id/replace`, `POST /api/themes/:id/lock` (EDIT) | Theme Editor, FontLibrary upload dropzone, FontReplacementModal, ColorEditorTab, FontEditorTab |
...
| `notifications` | `view-delivery-failures` | Authorizes viewing permanently failed email deliveries dashboard and receiving in-LMS failure alert notifications | `GET /api/notification-admin/delivery-failures` (HTTP route) & dynamic check via `permissionResolverService.filterUsersWithPermission` | Settings Delivery Failures card & DeliveryFailures page |
| `notifications` | `manage-rules` | Create, edit, enable/disable, duplicate, and soft-delete notification rules | `GET /api/notification-admin/rules`, `POST /api/notification-admin/rules`, `PATCH /api/notification-admin/rules/:id`, `DELETE /api/notification-admin/rules/:id`, `POST /api/notification-admin/rules/:id/duplicate` | Settings Notification Rules card & NotificationRuleManagement page |
| `notifications` | `manage-scheduled` | Create, edit, cancel, duplicate, and list scheduled company notifications | `GET /api/scheduled-notifications`, `POST /api/scheduled-notifications`, `PATCH /api/scheduled-notifications/:id`, `POST /api/scheduled-notifications/:id/cancel`, `POST /api/scheduled-notifications/:id/duplicate` | Settings Scheduled Notifications card & ScheduledNotificationManagement page |
| `notifications` | `manage-templates` | Create, edit, set default, soft-delete, and list company email HTML templates | `GET /api/email-templates`, `GET /api/email-templates/:id`, `POST /api/email-templates`, `PATCH /api/email-templates/:id`, `DELETE /api/email-templates/:id` | Settings Email Templates card, EmailTemplateManagement page, & EmailTemplateForm |
